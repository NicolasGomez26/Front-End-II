...
// const usuario = document.querySelector(".input-container")
const usuario = usuario.querySelectorAll("input")
const bd = JSON.parse(baseDeDatos);
...
form.addEventListener("submit", (e)=>{
  ...
  bd.forEach(e => {
    let emailIngresado = captarDataEmail()
    let passwordIngresada = captarDataPassword()
    if (emailIngresado == e.email && passwordIngresada == e.password) {
    location.replace = `./ejercicio-login-storage-main/sesionIniciada.html`
    const bienvenida = document.querySelector("h1")
      //   localStorage.setItem("sesion", JSON.stringify(baseDeDatos))
    // }
    else{alert("false")}
...
function captarDataEmail(){
  let data = email.value.trim();
  return data
}
function captarDataPassword() {
  let data = password.value.trim()
  return data
}
